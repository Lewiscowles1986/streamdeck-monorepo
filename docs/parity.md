...
Status: **2026-09-20** — all rows implemented and regression-tested (R7: multi-step sequence actions P18).
...
| P18 | Multi-step sequence actions. `execute()` recognizes `{"type": "sequence", "steps": [action, ...], "stopOnError": bool?}` and runs each step through the SAME internal execution path as a top-level action (`_execute_one` — steps may be command actions or nested sequences, nesting capped at depth 4; deeper yields a structured error result for that step, never a crash or runaway recursion). Aggregation: continue-on-error by DEFAULT (fire all steps; `"done"` if all succeeded, `"partial"` if some failed, `"failed"` if all failed — a P14 `"detached"` step counts as a SUCCESSFUL launch, not a failure); `stopOnError: true` aborts remaining steps on the first failure (`"failed"`, unexecuted steps reported `{"status": "skipped"}`). Each step may carry `delayMs` (sleep BEFORE that step, sanitized by `_as_delay_ms`: non-numeric/negative → 0, clamped at 60000). Template expansion recurses into steps with the same button context (`expand_template_vars` is already recursive over dicts/lists). Crash-safe: garbage sequences (missing/non-list `steps`, non-dict steps, corrupt step types) return structured failed results; the never-raise contract holds. Dispatched like commands: `dispatch_action` enqueues any dict action of type `command` **or** `sequence` (sequences are dict-only — no bare-string form, they need a payload). NO sequence-level timeout: per-step command `timeout`s already bound each step. UI: ActionEditor "Sequence (run multiple actions)" type — per-step compact cards (Executable/Arguments via CommandInput, launch mode, per-step "Delay before step (ms)", remove), sequence-level "Stop on Error" switch, "Add step", and an Advanced JSON fallback textarea for step types the card form does not host (a nested ActionEditor was deliberately not nested per-step). | `tests/test_agent.py` (sequence happy path, continue-on-error → partial, all-failed, stopOnError + marker-file proof, delayMs respected + clamp unit test, depth-2 nesting + depth-4 cap, garbage-sequence matrix, template expansion inside steps); `test_parity.py::test_sequence_action_reaches_agent_queue`; E2E: parity-demo "sequence action builds two steps and round-trips through JSON View", "sequence editor supports delay, stopOnError, and persists them", "steps can be removed from the sequence", parity-fullstack "sequence action with two steps round-trips through the real API" |
